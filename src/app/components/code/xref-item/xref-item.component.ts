...
@Component({
    ...
    template: `
    <div class="row g-0">
        <div [ngClass]="'col-lg-'+((item.tags!=null&& item.tags.length>0)?'10':'12')">
            <dxc-hook-status *ngIf="hookstatus" [item]="item"></dxc-hook-status>
            <dxc-icon [model]="xrefIcon"></dxc-icon>
            <dxc-node-token [interactive]="true" [item]="item"></dxc-node-token>
        </div>
...
export class XrefItemComponent implements OnInit, AfterViewInit {
    ...
    @Input() item?:any; //ModelMethod|ModelClass|ModelField;
